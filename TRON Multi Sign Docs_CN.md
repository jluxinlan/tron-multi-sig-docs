...
TRON 多签服务是一套面向应用层的多签交易服务，用于在不接触私钥的前提下，统一管理多签交易的权限校验、签名流程与执行控制。

开发者可通过该服务提交一笔待执行的链上交易，多签服务将依据账户预先配置的多签权限规则，对参与方的签名进行收集与校验，并以异步方式推进多签流程。 当累计签名权重满足链上账户的阈值要求时，系统将自动触发交易广播，完成链上执行。

TRON 多签服务不托管、不生成、不使用任何私钥，其定位是一个： 无私钥的多签交易中枢。

## 核心价值
标准化多签流程，支持异步签名收集与状态管理，自动判断签名权重与执行条件，避免业务方重复实现复杂多签逻辑，支持快速接入与扩展多签交易。
...
secret_id: SSSSSS （项目唯一标识）

secret_key: CCCCCCCC （签名密钥，需妥善保管）
```
### **3.3 备注**
为了方便项目方接入测试，提供一套key（有qps限制，不能频繁访问），如下：
```
channel: test
secret_id: TEST
...
主网域名：api.walletadapter.org

nile网域名：apinile.walletadapter.org
